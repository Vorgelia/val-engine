...
Font::Font(const FS::path& p)
{
	...
	if(!init)
	{
		...
		if(!init)
		{
			VE_LOG("Unable to init Freetype", LogItem::Type::Warning);
			return;
		}
	}
	...
	{
		VE_LOG("Unable to load font: " + name, LogItem::Type::Warning);
		return;
	}
	...
	for(GLubyte c = 0; c < 255; ++c)
	{
		if(FT_Load_Char(face, c, FT_LOAD_RENDER))
		{
			...
			VE_LOG("Unable to load character: " + c, LogItem::Type::Warning);
			continue;
		}
